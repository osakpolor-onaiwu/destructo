...
const datatype=['string','object','number','boolean','array']
...
class expects {
    ...
    Function(func){
      this.value.functions = func
      return this
    }
    Accept(args={}) {
        this.value.accepted_arguments = args
        
        return this;
    }
    Returns(type){
        this.value.returns_value = type
        
        function outputs(func,arg,returns){
        ...
        const {functions,accepted_arguments,returns_value} = this.value 
        const outputResult = outputs(functions,accepted_arguments,returns_value)
